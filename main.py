...
#             first=first['query'],  # or however you intend to pass the query
#             context=first['context'],
#             middle=middle[0],  # assuming middle is a list with one prompt template
#             last=last
#         )
...
def execute_research_query(chain, question: str):
    try:
        st.info(f"Attempting to invoke chain with query: {question}")
        
        # Try invoking the chain
        try:
            response = chain.invoke(question)
            st.info("Invoke successful")
            st.info("Raw response:", response)
            
            # Extract content
            content = response.content if hasattr(response, 'content') else str(response)
            
            st.info("Extracted content length:", len(content))
            print("Content preview:", content[:500])  # Print first 500 characters
            ...
            return {"output_text": f"Invoke error: {str(invoke_error)}"}
    ...
    # Usage example
    try:
        # Assuming you have exa_api_key and openai_api_key
        chain = create_research_chain(exa_api_key, openai_api_key)
        
        # Test invocation
        result = execute_research_query(chain, "Why has Apple's stock price increased?")
        print("Final result:", result)

    except Exception as e:
        print(f"Error creating or executing chain: {e}")
