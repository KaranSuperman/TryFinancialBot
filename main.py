...
def create_research_chain(exa_api_key: str, openai_api_key: str):
    # Initialize the search retriever with updated recency parameters
    retriever = ExaSearchRetriever(
        api_key=exa_api_key,
        k=10,  # Increased number of documents to fetch
        highlights=True,
        search_params={
            "recency_hours": 1,  # Fetch data from the last hour
            "use_autoprompt": True,
            "source_filters": {
                "include_domains": [
            ...
            "post_filter": {
                "min_word_count": 50  # Ensure substantive content
            }
        }
    )
    ...
    generation_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an expert in finance and stock market analysis. Your role is to provide the latest news, trends, and insights related to finance and stock markets, using the most up-to-date information available."),
        ("human", """
