...
def execute_research_query(chain, question: str):
    try:
        # Debug: Print out available secrets
        print("Available secrets:", list(st.secrets.keys()))
        print("Available 'news' keys:", list(st.secrets.get("news", {}).keys()))
        ...
        # Explicit validation
        if not exa_api_key:
            raise ValueError("Exa API key is missing from Streamlit secrets")
        if not openai_api_key:
            raise ValueError("OpenAI API key is missing from Streamlit secrets")
        
        print("EXA API Key Length:", len(exa_api_key))
        ...
        import traceback
        traceback.print_exc()
        return {"output_text": f"Configuration error: {str(e)}"}
